...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    parsed = np.array(line.split(','))
    labeledPoint = LabeledPoint(parsed[0], parsed[1:])
    return labeledPoint
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return sqrt(labelsAndPreds.map(lambda x: squaredError(x[0], x[1])).sum()/labelsAndPreds.count())
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPreds = parsedValData.map(lambda x: (x.label, firstModel.predict(np.array(x.features)) ))
...
regParam = np.array([1e-10, 1e-5, 1])
